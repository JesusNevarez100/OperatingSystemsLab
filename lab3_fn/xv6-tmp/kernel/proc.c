...
{
  struct proc *p;
  struct proc *best; // <--- Holds best nice value's
  ...
  for(;;){
    ...
    // find proc with lowest nice value, must be runnable
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE) {
...
int
set_priority_by_pid(int pid, int prio)
{
  struct proc *p;
  for(p = proc; p < &proc[NPROC]; p++){
    acquire(&p->lock);
